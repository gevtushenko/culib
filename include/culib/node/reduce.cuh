...
template <typename data_type, class sync_policy=atomic_threads_synchronizer>
class reducer
{
  data_type *gpu_workspace {};
  const culib::node::device_communicator<sync_policy> &communicator;

public:
  reducer () = delete;
  reducer (
    data_type *gpu_workspace_arg,
    const culib::node::device_communicator<sync_policy> &communicator_arg)
    ...
    const data_type *input)
  {
    ...
    return communicator.host_reduce_sum (device_reducer.reduce_from_host (current_gpu_elements_count, input));
  }

  static size_t get_gpu_workspace_size (size_t elements_count)
  {
    return culib::device::reducer<data_type>::get_gpu_workspace_size (elements_count);
  }
